...
- **`Renderer`** (`crates/tv-ui/src/renderer.rs`) — the pluggable draw backend (primitives only: lines, circles, triangles, `draw_image`, `draw_images`, `draw_text`; `fill_rect`/`stroke_rect` are default compositions, plus `set_clip`, `prefetch_image`, and `draw_image_cached` / `draw_images_cached` for motion). UI never touches GL directly. Implemented by `WebGl2Renderer` (`crates/tv-ui-webgl/src/webgl2.rs`).
...
`MainShell::new(brand, tab_labels)` owns the `NavBar` plus a lazily-populated `Vec<Option<CatalogPage>>` (one per label) and a `Tween` `slide` that horizontally translates the active page into view when the tab changes. It clips each page strip with `set_clip` so neighbours don't bleed during the slide. `tv-app` supplies its own brand/tabs (`"WASM TV"`, `Home`/`Movies`/`Shows`) at the one production call site in `crates/tv-app/src/lib.rs`; nothing about them is baked into `tv-ui`.
...
Rails lazy-reveal in batches of `RAIL_BATCH` (5) as focus nears the end of what's loaded. `Enter` on a card/banner opens the `MetadataOverlay`; `Enter` on the overlay reports `Transition::Activate(ActivatedItem { id, title, image_url })` — the overlay only knows the card's own render data, not what "activating" means, so it hands the event upward and lets the driving app decide (`tv-app`'s `App::handle_key` resolves `id` to a video URL via `content::video_url_for` and pushes `PlayerScreen`). This mirrors `tv-ui-web`'s `select`/`focuschange` `CustomEvent`s: components report, the host app decides.

`Metrics::default()` (`crates/tv-ui/src/metrics.rs`) is the single knob for card size / spacing / band heights / rail-title and card-name font sizes (`Layout` is a back-compat type alias for `Metrics`); its values match `tv-app`'s original tuning, but any consumer (e.g. `tv-ui-web`'s smaller `embed_metrics()`) can override every field. Pure geometry (`Rect`/`Size`/`Insets`) lives in `crates/tv-ui/src/geom.rs`. Design space is no longer a `tv-ui` constant — each app owns its own `DESIGN_WIDTH`/`DESIGN_HEIGHT` locals and threads the resulting `Rect` in via `Ctx.design`.
...
`WebGl2Renderer` (`crates/tv-ui-webgl/src/webgl2.rs`) batches vector primitives into two vertex `Vec<f32>` buffers (triangles + lines) and flushes them on `end_frame`, before any textured work, and on `set_clip` (to preserve draw call order). Text is rasterized on an offscreen 2D canvas and cached by `size|color|text`; banner images use a per-URL `TEXTURE_2D` LRU, while card rails use `draw_images` against a fixed-size `TEXTURE_2D_ARRAY` (layer WxH from the host's card metrics) with instanced quads. Decoded sources live in an async LRU `ImageCache`. New GPU uploads are capped per frame via `WebGl2RendererConfig::image_uploads_per_frame` (default 2). During vertical rail motion, `RailList` draws with `draw_images_cached` (resident only) and prefetches; full `draw_images` resumes once settled. The banner keeps `draw_image` during page motion (budget-amortized) and uses a slightly slower `BANNER_TAU` so hold-chaining does not outrun full-bleed uploads. `set_clip` maps a design-space `Rect` to a GL scissor box. See [`doc/RENDERING.md`](doc/RENDERING.md).
