...
`WebGl2Renderer` (`crates/tv-ui-webgl/src/webgl2.rs`) batches vector primitives into two vertex `Vec<f32>` buffers (triangles + lines) and flushes them on `end_frame`, before any textured work, and on `set_clip` (to preserve draw call order). Text is rasterized on an offscreen 2D canvas and cached by `size|color|text`; banner images use a per-URL `TEXTURE_2D` LRU, while card rails use `draw_images` against a fixed-size `TEXTURE_2D_ARRAY` (layer WxH from the host's card metrics) with instanced quads. Decoded sources live in an async LRU `ImageCache`. New GPU uploads are capped per frame via `WebGl2RendererConfig::image_uploads_per_frame` (default 2), so `RailList` can keep `draw_images` during vertical motion without hitching; `draw_images_cached` remains available for resident-only draws. `set_clip` maps a design-space `Rect` to a GL scissor box. See [`doc/RENDERING.md`](doc/RENDERING.md).
