...
`WebGl2Renderer` (`crates/tv-ui-webgl/src/webgl2.rs`) batches vector primitives into two vertex `Vec<f32>` buffers (triangles + lines) and flushes them on `end_frame`, before any textured work, and on `set_clip` (to preserve draw call order). Text is rasterized on an offscreen 2D canvas and cached by `size|color|text`; banner images use a per-URL `TEXTURE_2D` LRU, while card rails use `draw_images` against a fixed-size `TEXTURE_2D_ARRAY` (layer WxH from the host's card metrics) with instanced quads. Decoded sources live in an async LRU `ImageCache`. During rail motion, `RailList` prefetches nearby posters and draws with `draw_images_cached` (array layers already resident only) to avoid upload hitching; full `draw_images` resumes once the tween settles. `set_clip` maps a design-space `Rect` to a GL scissor box. See [`doc/RENDERING.md`](doc/RENDERING.md).
